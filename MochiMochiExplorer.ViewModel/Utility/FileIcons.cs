...
namespace MochiMochiExplorer.ViewModel.Wpf.Utility
{
    internal class FileIcons
    {
        ...
        public async Task<BitmapSource?> GetAssociatedIcon(string inFilepath, CancellationToken? inCancellationToken = null)
            => await GetAssociatedIcon(inFilepath, IconSize.Small, inCancellationToken);

        public async Task<BitmapSource?> GetAssociatedIcon(string inFilepath, IconSize inSize, CancellationToken? inCancellationToken = null)
        {
            using (await _mutex.LockAsync())
            {
                if (_uniqueIcons.TryGetValue(inFilepath, out var result))
                    return result;

                result = null;
                var iconIndex = GetFileIconIndex(inFilepath);
                if (iconIndex != 0)
                {
        ...
        private BitmapSource CreateAssociateIconBitmap(string inFilepath, IconSize inSize)
        {
            ...
            if (iconIndex == 0)
            {
                var icon = Icon.ExtractAssociatedIcon(inFilepath);
                if (icon is null)
                    throw new Exception($"Failed to extract associate icon from \"{inFilepath}\".");
                ...
                _uniqueIcons.Add(inFilepath, result);
            }
            else
            {
        ...
        private SHFILEINFO GetFileInfo(string inFilepath)
        {
            if (!File.Exists(inFilepath))
                throw new FileNotFoundException();
            ...
            if (SHGetFileInfo(inFilepath, 0, ref result, (uint)Marshal.SizeOf(result), flags) == IntPtr.Zero)
